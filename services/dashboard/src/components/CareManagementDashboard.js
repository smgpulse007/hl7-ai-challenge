          ...
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 'bold' }}>
                Care Management: {selectedMember.name}
              </h2>
              <button
                style={{
                  ...styles.button,
            ...
            <div style={{ marginBottom: '1.5rem', padding: '1rem', backgroundColor: '#f9fafb', borderRadius: '0.5rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <strong>Member ID:</strong> {selectedMember.member_id}<br/>
                  <strong>Age:</strong> {selectedMember.age}<br/>
                  <strong>Language:</strong> {selectedMember.language}<br/>
                  <strong>Measure:</strong> {selectedMember.measure_type}
                </div>
                <div>
                  <strong>Risk Level:</strong> <span style={{ color: getRiskColor(selectedMember.risk_level) }}>
                    {selectedMember.risk_level}
                  </span><br/>
                  <strong>Risk Probability:</strong> {(selectedMember.risk_probability * 100).toFixed(1)}%<br/>
                  <strong>Due Date:</strong> {selectedMember.intervention_due_date}<br/>
                  <strong>Care Coordinator:</strong> {selectedMember.care_features?.care_coordinator}
                </div>
              </div>
            </div>
            ...
            {selectedMember.clinical_quote && (
              <div style={{ marginBottom: '1.5rem', padding: '1rem', backgroundColor: '#f0f9ff', borderRadius: '0.5rem' }}>
                <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.125rem' }}>Clinical Evidence - RAG Pipeline Extraction</h3>
                <div style={{ marginBottom: '1rem', padding: '0.75rem', backgroundColor: '#e0f2fe', borderRadius: '0.375rem', border: '1px solid #0284c7' }}>
                  <strong style={{ color: '#0284c7' }}>Extracted Clinical Quote:</strong>
                  <div style={{ marginTop: '0.5rem', fontStyle: 'italic', color: '#0f172a', fontSize: '0.95rem' }}>
                    "{selectedMember.clinical_quote}"
                  </div>
                  <div style={{ marginTop: '0.5rem', fontSize: '0.75rem', color: '#64748b' }}>
                    Source: {getEvidenceSourceDisplay(selectedMember.evidence_source).label} |
                    Processing Time: {selectedMember.processing_time}s |
                    Method: LLaMA 3.2 + spaCy NER
                  </div>
                </div>
              </div>
            )}

            {/* Model Features & Risk Analysis */}
            {selectedMember.model_features && (
              <div style={{ marginBottom: '1.5rem', padding: '1rem', backgroundColor: '#fef7ff', borderRadius: '0.5rem' }}>
                <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.125rem' }}>XGBoost Model Features</h3>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', fontSize: '0.875rem' }}>
                  <div>
                    <strong>Healthcare Utilization:</strong><br/>
                    • PCP Visits 2023: {selectedMember.model_features.pcp_visits_2023 || 0}<br/>
                    • Chronic Conditions: {selectedMember.model_features.chronic_condition_count || 0}<br/>
                    • ED Visits: {selectedMember.model_features.ed_visits_2023 || 0}<br/>
                    • Immunizations: {selectedMember.model_features.immunizations_2023 || 0}
                  </div>
                  <div>
                    <strong>Engagement Patterns:</strong><br/>
                    • PCP Assigned: {selectedMember.model_features.has_pcp_assigned ? 'Yes' : 'No'}<br/>
                    • Engagement Level: {selectedMember.model_features.pcp_engagement_level || 'Unknown'}<br/>
                    • Utilization Pattern: {selectedMember.model_features.utilization_pattern || 'Unknown'}<br/>
                    • English Primary: {selectedMember.model_features.english_primary ? 'Yes' : 'No'}
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
              {selectedMember.risk_factors && selectedMember.risk_factors.length > 0 && (
                <div style={{ padding: '1rem', backgroundColor: '#fef2f2', borderRadius: '0.5rem' }}>
                  <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.125rem', color: '#dc2626' }}>Risk Factors</h3>
                  <ul style={{ margin: 0, paddingLeft: '1.5rem' }}>
                    {selectedMember.risk_factors.map((factor, index) => (
                      <li key={index} style={{ marginBottom: '0.25rem', color: '#dc2626' }}>{factor}</li>
                    ))}
                  </ul>
                </div>
              )}
              ...
              {selectedMember.protective_factors && selectedMember.protective_factors.length > 0 && (
                <div style={{ padding: '1rem', backgroundColor: '#f0fdf4', borderRadius: '0.5rem' }}>
                  <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.125rem', color: '#16a34a' }}>Protective Factors</h3>
                  <ul style={{ margin: 0, paddingLeft: '1.5rem' }}>
                    {selectedMember.protective_factors.map((factor, index) => (
                      <li key={index} style={{ marginBottom: '0.25rem', color: '#16a34a' }}>{factor}</li>
                    ))}
                  </ul>
                </div>
              )}
            ...
            {/* Next Appointments */}
            {selectedMember.next_appointments && selectedMember.next_appointments.length > 0 && (
              <div style={{ marginBottom: '1.5rem', padding: '1rem', backgroundColor: '#f0fdf4', borderRadius: '0.5rem' }}>
                <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.125rem' }}>Next Appointments</h3>
                {selectedMember.next_appointments.map((apt, index) => (
                  <div key={index} style={{ marginBottom: '0.5rem' }}>
                    <strong>{apt.date}</strong> - {apt.type} with {apt.provider} ({apt.status})
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              <button
                ...
                onClick={() => sendManualAlert(selectedMember.member_id, 'reminder', 'Care gap reminder')}
              >
                Send Manual Alert
              </button>
              
              {selectedMember.gap_closure_available && (
                <button
                  style={{
                    ...styles.button,
                  ...
                  onClick={() => closeGap(selectedMember.member_id, selectedMember.measure_type, 'Evidence found', 'Manual review')}
                >
                  Close Gap in MedHok
                </button>
              )}
              ...
              <button
                ...
                onClick={() => alert('Schedule appointment functionality would integrate with scheduling system')}
              >
